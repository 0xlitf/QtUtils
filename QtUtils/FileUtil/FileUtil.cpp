...
namespace QtUtils
{
	...
	QList<QString> FileUtil::scanAllFiles(const QString& folder)
	{
		QList<QString> list;

		for (auto d : QDir(folder).entryInfoList(QDir::NoDotAndDotDot | QDir::Dirs | QDir::Files))
		{
			if (d.isDir())
			{
				list.append(scanAllFiles(d.absoluteFilePath()));
			}
			else
			{
				list.append(d.absoluteFilePath());
			}
		}
